...
for(i in 1:(nspec)){
	...
	z[i,k,1] ~ dbern(psi_in[i,k])
		...
		logit(C[i,k,t-1]) <- gam0[i,t-1]  + 
					gam_d[i] * sum(exp(-(inprod(alpha_open_vec[,i], (open_dmat[sm[,k],k] * z[i,sm[,k],t-1])) + 
							  inprod(alpha_dvlp_high_vec[,i], (dvlp_high_dmat[sm[,k],k] * z[i,sm[,k],t-1])) +
							  inprod(alpha_dvlp_mid_vec[,i], (dvlp_mid_dmat[sm[,k],k] * z[i,sm[,k],t-1])) +
							  inprod(alpha_green_vec[,i], (green_dmat[sm[,k],k] * z[i,sm[,k],t-1])) +
							  inprod(alpha_water_vec[,i], (water_dmat[sm[,k],k] * z[i,sm[,k],t-1])) + 
							  inprod(alpha_ag_vec[,i], (ag_dmat[sm[,k],k] * z[i,sm[,k], t-1])))))
			# the extinction part
